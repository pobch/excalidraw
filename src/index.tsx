...
function isTextElement(element: ExcaliburElement): element is ExcaliburTextElement {
        ...
        <canvas
          ...
          onMouseDown={(e) => {
            ...
            if (this.state.elementType === 'selection') {
              ...
              // * Is true when we click-and-drag on a SINGLE element
            ...
            if (this.state.elementType === 'text') {
              // ! This line force switching to "selection" tool without triggering
              // ! ... onMouseUp() to clear eventListener
              this.setState({
                draggingElement: null,
                elementType: 'selection',
              })
            ...
            const onMouseMove = (e: MouseEvent) => {
              ...
              if (isDraggingElements) {
                // * Reach here only when we select a single element

                const selectedElements = elements.filter((el) => el.isSelected) // * Will always have length === 1
                if (selectedElements.length) {
                  ...
                  lastY = y
                  this.forceUpdate()
                  // * Skip drawing a rectangle selection area
                  return
                }
              }
              ...
              // * Moving mouse without holding click
              if (!draggingElement) return

              // * All tools while holding click and drag. P.s. "text" cannot click-and-drag because of prompt()
              let width = e.clientX - target.offsetLeft - draggingElement.x
              let height = e.clientY - target.offsetTop - draggingElement.y
              // ! Mutate state without setState
              ...
              if (this.state.elementType === 'selection') {
                // * Mutate each element.isSelected to `true` if they are inside the rectangle selection area
            ...
            const onMouseUp = (e: MouseEvent) => {
              // * Because of prompt(), "text" tool will never get here(onMouseUp() is never triggered)
              ...
              if (draggingElement === null) {
                // ! Actually get triggered on edge case where there are two onMouseUp listeners
                // ! ... because "text" tool forces switching to "selection" tool
                clearSelection()
                this.forceUpdate()
                return
              }
              ...
              if (elementType === 'selection') {
                ...
                // ! Work because mutating `draggingElement` also mutate `element`
            ...
            // ! onMouseDown is on <canvas/>, whereas onMouseMove and onMouseUp are on window
